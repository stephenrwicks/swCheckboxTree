type TCheckboxTree = {
    ...
    options: Array<TOption>; // setter / getter
}
type TOption = {
...
function swCheckboxTree(options?: Array<TOption>): TCheckboxTree {
    const recursiveBuild = (options: Array<TOption>, parent?: HTMLDivElement): TCheckboxTree => {
        const tree: TCheckboxTree = {
            options: options,
        ...
        if (parent) parent.append(tree.element);
        return tree;
    };
    ...
    const handleChildTreeChange = (childTree: TCheckboxTree, parentOption: TOption) => {
        if (!parentOption.element) return;
    ...
    const recursiveFlatten = (options: Array<TOption>) => {
        const flatResult: Array<TOption> = [];
        for (const option of options) {
            flatResult.push(option);
            if (option.children?.length) flatResult.push(...recursiveFlatten(option.children));
        }
        return flatResult;
    };
    // Make a param?
    const _options: Array<TOption> = [];
    return {
        ...
        set options(options: Array<TOption>) {
